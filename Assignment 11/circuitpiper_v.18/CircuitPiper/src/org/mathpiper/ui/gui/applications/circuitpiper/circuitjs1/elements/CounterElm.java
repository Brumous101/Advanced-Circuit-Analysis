...
public class CounterElm extends ChipElm {
    ...
    public void setupPins() {
        ...
        pins[0].clock = true;
        pins[1] = new Pin(sizeY - 1, SIDE_WEST, "R");
        pins[1].bubble = invertreset;
        int i;
        for (i = 0; i != bits; i++) {
            int ii = i + 2;
            pins[ii] = new Pin(i, SIDE_EAST, "Q" + (bits - i - 1));
            pins[ii].output = pins[ii].state = true;
        }
        if (hasUpDown()) {
            pins[bits + 2] = new Pin(sizeY - 2, SIDE_WEST, "U/D");
        }
        allocNodes();
    }
    ...
    public EditInfo getEditInfo(int n) {
        ...
        if (n == 5) {
            EditInfo ei = new EditInfo("", 0, -1, -1);
            ei.checkbox = new JCheckBox("Up/Down Pin", hasUpDown());
            return ei;
        }
        return null;
    }
    ...
    public void setEditValue(int n, EditInfo ei) {
        ...
        if (n == 2) {
            invertreset = ei.checkbox.isSelected();
            setupPins();
            setPoints();
        }
        if (n == 3 && ei.value >= 3) {
