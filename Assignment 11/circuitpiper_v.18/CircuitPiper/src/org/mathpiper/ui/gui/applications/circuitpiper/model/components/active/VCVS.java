...
public class VCVS extends ControlledSource {
    ...
    public VCVS(int x, int y, String uid, Circuit circuit) {
    ...
    public VCVS(int x, int y, String uid, Stack<String> attributes, Circuit circuit) throws Exception {
        super(x, y, circuit);
        if(uid == null)
        {
            componentSubscript = componentCounter++ + "";
    ...
    public void init() {
        ...
        setPrimarySymbol("VCVS");
        setSiPrefix("");
        setPrimaryUnitSymbol("V");
        setPreselectedPrimaryPrefix(getSiPrefix() + getPrimaryUnitSymbol());
        controllerUnitSymbol = "`V";
        setLabel(multiplier + "*???" + controllerUnitSymbol);
    }    
