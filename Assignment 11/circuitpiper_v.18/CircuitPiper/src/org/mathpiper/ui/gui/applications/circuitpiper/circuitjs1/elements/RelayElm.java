...
public class RelayElm extends CircuitElm {
    ...
    public void draw(Graphics g) {
        int i, p;
        for (i = 0; i != 2; i++) {
            setVoltageColor(g, nodeVoltages[nCoil1 + i]);
            drawThickLine(g, coilLeads[i], coilPosts[i]);
        }
        int x = ((flags & FLAG_SWAP_COIL) != 0) ? 1 : 0;
        setPowerColor(g, coilCurrent * (nodeVoltages[nCoil1] - nodeVoltages[nCoil2]));
        drawCoil(g, dsign * 6, coilLeads[x], coilLeads[1 - x],
                nodeVoltages[nCoil1 + x], nodeVoltages[nCoil2 - x]);
    ...
    public void startIteration() {
        ind.startIteration(nodeVoltages[nCoil1] - nodeVoltages[nCoil3]);
    ...
    public void doStep() {
        double voltdiff = nodeVoltages[nCoil1] - nodeVoltages[nCoil3];
        ind.doStep(voltdiff);
        int p;
    ...
    void calculateCurrent() {
        ...
        for (p = 0; p != poleCount; p++) {
            if (i_position == 2) {
                ...
                switchCurrent[p]
                        = (nodeVoltages[nSwitch0 + p * 3] - nodeVoltages[nSwitch1 + p * 3 + i_position]) / r_on;
            }
        }
    }
    ...
    public void getInfo(String arr[]) {
        ...
        arr[ln++] = "coil Vd = "
                + getVoltageDText(nodeVoltages[nCoil1] - nodeVoltages[nCoil2]);
    }
