...
public class InvertingSchmittElm extends CircuitElm {
    ...
    public InvertingSchmittElm(int xa, int ya, int xb, int yb, int f,
        ...
        try {
            ...
            logicOnLevel = Double.parseDouble(st.nextToken());
            logicOffLevel = Double.parseDouble(st.nextToken());
        } catch (Exception e) {
        }
    }
    ...
    public void doStep() {
        double v0 = nodeVoltages[1];
        double out;
        if (state) {//Output is high
            if (nodeVoltages[0] > upperTrigger)//Input voltage high enough to set output low
            {
                state = false;
                out = logicOffLevel;
            } else {
            ...
            if (nodeVoltages[0] < lowerTrigger)//Input voltage low enough to set output high
            {
                state = true;
                out = logicOnLevel;
            } else {
    ...
    public void getInfo(String arr[]) {
        ...
        arr[1] = "Vi = " + getVoltageText(nodeVoltages[0]);
        arr[2] = "Vo = " + getVoltageText(nodeVoltages[1]);
    }
