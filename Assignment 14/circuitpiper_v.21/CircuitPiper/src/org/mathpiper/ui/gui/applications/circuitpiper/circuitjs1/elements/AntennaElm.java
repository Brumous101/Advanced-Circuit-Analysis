...
public class AntennaElm extends RailElm {
    ...
    double getVoltage() {
        fmphase += 2 * pi * (2200 + Math.sin(2 * pi * sim.simulationTime * 13) * 100) * sim.timeStep;
        double fm = 3 * Math.sin(fmphase);
        return Math.sin(2 * pi * sim.simulationTime * 3000) * (1.3 + Math.sin(2 * pi * sim.simulationTime * 12)) * 3
                + Math.sin(2 * pi * sim.simulationTime * 2710) * (1.3 + Math.sin(2 * pi * sim.simulationTime * 13)) * 3
                + Math.sin(2 * pi * sim.simulationTime * 2433) * (1.3 + Math.sin(2 * pi * sim.simulationTime * 14)) * 3 + fm;
    }
