...
public class VoltageElm extends CircuitElm {
    ...
    public static final double defaultPulseDuty = 1 / (2 * Math.PI);
    ...
    double getVoltage() {
        ...
        double w = 2 * pi * (sim.simulationTime - freqTimeZero) * frequency + phaseShift;
        switch (waveform) {
            case WF_DC:
    ...
    public EditInfo getEditInfo(int n) {
        ...
        if (n == 1) {
            ...
            ei.comboBox.addItem("D/C");
            ei.comboBox.addItem("A/C");
            ei.comboBox.addItem("Square Wave");
            ei.comboBox.addItem("Triangle");
            ei.comboBox.addItem("Sawtooth");
            ei.comboBox.addItem("Pulse");
            ei.comboBox.addItem("Noise");
            ei.comboBox.setSelectedItem(waveform);
            return ei;
        }
        if (n == 2) {
    ...
    public void setEditValue(int n, EditInfo ei) {
        ...
        if (n == 3) {
            ...
            if (frequency > maxfreq) {
                if (JOptionPane.showConfirmDialog(null, sim.LS("Adjust timestep to allow for higher frequencies?")) == JOptionPane.YES_OPTION) {
            ...
            freqTimeZero = sim.simulationTime - oldfreq * (sim.simulationTime - freqTimeZero) / frequency;
        }
        if (n == 1) {
            int ow = waveform;
            waveform = ei.comboBox.getSelectedIndex();
