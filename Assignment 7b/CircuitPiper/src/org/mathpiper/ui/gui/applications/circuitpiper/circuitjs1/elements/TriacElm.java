...
public class TriacElm extends CircuitElm {
    ...
    public void reset() {
        nodeVoltages[mt1node] = nodeVoltages[mt2node] = nodeVoltages[gnode] = 0;
        diode03.reset();
        diode30.reset();
    ...
    public void doStep() {
        diode03.doStep(nodeVoltages[mt2node] - nodeVoltages[mtinode]);
        diode30.doStep(nodeVoltages[mtinode] - nodeVoltages[mt2node]);
    ...
    public void getInfo(String arr[]) {
        arr[0] = "TRIAC";
        arr[1] = (state) ? "on" : "off";
        arr[2] = "Vmt2mt1 = " + getVoltageText(nodeVoltages[mt2node] - nodeVoltages[mt1node]);
    ...
    void calculateCurrent() {
        i2 = (nodeVoltages[mtinode] - nodeVoltages[mt1node]) / aresistance;
        ig = -(nodeVoltages[mt1node] - nodeVoltages[gnode]) / cresistance;
        i1 = -i2 - ig;
    }

    double getPower() {
        return (nodeVoltages[mt2node] - nodeVoltages[mt1node]) * i2 + (nodeVoltages[gnode] - nodeVoltages[mt1node]) * ig;
    }
