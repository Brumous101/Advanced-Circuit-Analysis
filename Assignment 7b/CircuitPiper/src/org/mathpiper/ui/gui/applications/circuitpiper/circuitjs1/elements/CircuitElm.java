...
//	int ypd = b.y-a.y;
    ...
    public void drawDots(ScaledGraphics g, Point pb, Point pa, double pos) {
        if ((!sim.simIsRunning()) || pos == 0 || !sim.isDots) {
            return;
        }
        int dx = pb.x - pa.x;
    ...
    // Note:tk:the head and the tail have been reversed here because CircuitPiper draws components
    // head to tail while CirSim draws components tail to head.
    public void drawDots(ScaledGraphics g, double x2, double y2, double x1, double y1, double pos) {
...
//	drawThickLine(g, xs[i], ys[i], xs[0], ys[0]);
    ...
    public void doDots(ScaledGraphics g) {
        updateDotCount();
        if (sim.dragElm != this) {
            drawDots(g, point1, point2, curcount);
        }
