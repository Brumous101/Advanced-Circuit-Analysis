...
public class SCRElm extends CircuitElm {
    ...
    public SCRElm(int xa, int ya, int xb, int yb, int f,
        ...
        try {
            ...
            nodeVoltages[anode] = 0;
            nodeVoltages[cnode] = -lastvac;
            nodeVoltages[gnode] = -lastvag;
    ...
    public void reset() {
        nodeVoltages[anode] = nodeVoltages[cnode] = nodeVoltages[gnode] = 0;
        diode.reset();
    ...
    public String dump() {
        return super.dump() + " " + (nodeVoltages[anode] - nodeVoltages[cnode]) + " "
                + (nodeVoltages[anode] - nodeVoltages[gnode]) + " " + triggerI + " " + holdingI + " "
                + cresistance;
    }
    ...
    public void draw(Graphics g) {
        ...
        double v1 = nodeVoltages[anode];
        double v2 = nodeVoltages[cnode];
        ...
        setVoltageColor(g, nodeVoltages[gnode]);
    ...
    double getPower() {
        return (nodeVoltages[anode] - nodeVoltages[gnode]) * ia + (nodeVoltages[cnode] - nodeVoltages[gnode]) * ic;
    }
    ...
    public void doStep() {
        double vac = nodeVoltages[anode] - nodeVoltages[cnode]; // typically negative
        double vag = nodeVoltages[anode] - nodeVoltages[gnode]; // typically positive
        ...
        lastvac = vac;
        lastvag = vag;
        diode.doStep(nodeVoltages[inode] - nodeVoltages[gnode]);
        ...
        //System.out.println(vac + " " + vag + " " + sim.converged + " " + ic + " " + ia + " " + aresistance + " " + nodeVoltages[inode] + " " + nodeVoltages[gnode] + " " + nodeVoltages[anode]);
        sim.stampResistor(nodes[anode], nodes[inode], aresistance);
    }

    public void getInfo(String arr[]) {
        arr[0] = "SCR";
        double vac = nodeVoltages[anode] - nodeVoltages[cnode];
        double vag = nodeVoltages[anode] - nodeVoltages[gnode];
        double vgc = nodeVoltages[gnode] - nodeVoltages[cnode];
    ...
    void calculateCurrent() {
        ic = (nodeVoltages[cnode] - nodeVoltages[gnode]) / cresistance;
        ia = (nodeVoltages[anode] - nodeVoltages[inode]) / aresistance;
        ig = -ic - ia;
    }
