...
import org.mathpiper.ui.gui.applications.circuitpiper.circuitjs1.Inductor;
...
public class TransformerElm extends CircuitElm {
    ...
    public void draw(Graphics g) {
        ...
        for (i = 0; i != 2; i++) {
            setPowerColor(g, current[i] * (nodeVoltages[i] - nodeVoltages[i + 2]));
            drawCoil(g, dsign * (i == 1 ? -6 * polarity : 6), ptCoil[i], ptCoil[i + 2], nodeVoltages[i], nodeVoltages[i + 2]);
        }
    ...
    public void reset() {
        ...
        current[0] = current[1] = nodeVoltages[0] = nodeVoltages[1] = nodeVoltages[2]
                = nodeVoltages[3] = curcount[0] = curcount[1] = curSourceValue1 = curSourceValue2 = 0;
    }
    double a1, a2, a3, a4;
    ...
    public void startIteration() {
        double voltdiff1 = nodeVoltages[0] - nodeVoltages[2];
        double voltdiff2 = nodeVoltages[1] - nodeVoltages[3];
        if (isTrapezoidal()) {
    ...
    public void getInfo(String arr[]) {
        ...
        arr[2] = "Ratio = 1:" + ratio;
        arr[3] = "Vd1 = " + getVoltageText(nodeVoltages[0] - nodeVoltages[2]);
        arr[4] = "Vd2 = " + getVoltageText(nodeVoltages[1] - nodeVoltages[3]);
    ...
    public EditInfo getEditInfo(int n) {
        ...
        if (n == 4) {
            EditInfo ei = new EditInfo("", 0, -1, -1);
            ei.checkbox = new JCheckBox("Swap Secondary Polarity",
                    polarity == -1);
            return ei;
        }
        return null;
    ...
    public void setEditValue(int n, EditInfo ei) {
        ...
        if (n == 4) {
            polarity = (ei.checkbox.isSelected()) ? -1 : 1;
            if (ei.checkbox.isSelected()) {
                flags |= FLAG_REVERSE;
            } else {
                flags &= ~FLAG_REVERSE;
