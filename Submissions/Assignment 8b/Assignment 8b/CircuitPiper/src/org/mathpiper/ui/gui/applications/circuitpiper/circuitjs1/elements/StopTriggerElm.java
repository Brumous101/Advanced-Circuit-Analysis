...
public class StopTriggerElm extends CircuitElm {
    ...
    public void stepFinished() {
        stopped = false;
        if (!triggered && ((type == 0 && nodeVoltages[0] >= triggerVoltage) || (type == 1 && nodeVoltages[0] <= triggerVoltage))) {
            triggered = true;
            triggerTime = sim.simulationTime;
        }
        if (triggered && sim.simulationTime >= triggerTime + delay) {
            triggered = false;
            stopped = true;
    ...
    public void getInfo(String arr[]) {
        ...
        arr[3] = (triggered) ? ("stopping in " + getUnitText(triggerTime + delay - sim.simulationTime, "s")) : (stopped) ? "stopped" : "waiting";
    }
    ...
    public EditInfo getEditInfo(int n) {
        ...
        if (n == 1) {
            ...
            ei.comboBox.addItem(">=");
            ei.comboBox.addItem("<=");
            ei.comboBox.setSelectedItem(type);
            return ei;
        }
        if (n == 2) {
    ...
    public void setEditValue(int n, EditInfo ei) {
        ...
        if (n == 1) {
            type = ei.comboBox.getSelectedIndex();
        }
        if (n == 2) {
            delay = ei.value;
        }
