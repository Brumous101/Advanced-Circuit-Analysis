...
public class MosfetElm extends CircuitElm {
    ...
    public MosfetElm(int xa, int ya, int xb, int yb, int f,
        ...
        allocNodes(); // make sure nodeVoltages[] has the right number of elements when hasBodyTerminal() is true 
    }
    ...
    public void reset() {
        lastv1 = lastv2 = nodeVoltages[0] = nodeVoltages[1] = nodeVoltages[2] = curcount = 0;
        diodeB1.reset();
        diodeB2.reset();
    }
    ...
    public void draw(Graphics g) {
        ...
        for (i = 0; i != segments; i++) {
            ...
            double v = nodeVoltages[1] + (nodeVoltages[2] - nodeVoltages[1]) * i / segments;
            if (!power) {
                setVoltageColor(g, v);
            }
        ...
        if (showBulk()) {
            setVoltageColor(g, nodeVoltages[bodyTerminal]);
            if (!hasBodyTerminal()) {
    ...
    double getPower() {
        return ids * (nodeVoltages[2] - nodeVoltages[1]) - diodeCurrent1 * (nodeVoltages[1] - nodeVoltages[bodyTerminal]) - diodeCurrent2 * (nodeVoltages[2] - nodeVoltages[bodyTerminal]);
    }
    ...
    void calculate(boolean finished) {
        ...
        if (doBodyDiode()) {
            diodeB1.doStep(pnp * (nodeVoltages[bodyTerminal] - nodeVoltages[1]));
            diodeCurrent1 = diodeB1.calculateCurrent(pnp * (nodeVoltages[bodyTerminal] - nodeVoltages[1])) * pnp;
            diodeB2.doStep(pnp * (nodeVoltages[bodyTerminal] - nodeVoltages[2]));
            diodeCurrent2 = diodeB2.calculateCurrent(pnp * (nodeVoltages[bodyTerminal] - nodeVoltages[2])) * pnp;
        } else {
        ...
        //System.out.println("M " + vds + " " + vgs + " " + ids + " " + gm + " "+ Gds + " " + nodeVoltages[0] + " " + nodeVoltages[1] + " " + nodeVoltages[2] + " " + source + " " + rs + " " + this);
    ...
    @SuppressWarnings("public static-access")
    void getFetInfo(String arr[], String n) {
        ...
        arr[2] = "Vgs = " + getVoltageText(nodeVoltages[0] - nodeVoltages[pnp == -1 ? 2 : 1]);
        arr[3] = ((pnp == 1) ? "Vds = " : "Vsd = ") + getVoltageText(nodeVoltages[2] - nodeVoltages[1]);
    ...
    public double getVoltageDiff() {
        return nodeVoltages[2] - nodeVoltages[1];
    }
    ...
    public EditInfo getEditInfo(int n) {
        ...
        if (n == 2) {
            ...
            ei.checkbox = new JCheckBox("Show Bulk", showBulk());
            return ei;
        }
        if (n == 3) {
            EditInfo ei = new EditInfo("", 0, -1, -1);
            ei.checkbox = new JCheckBox("Swap D/S", (flags & FLAG_FLIP) != 0);
            return ei;
        }
        if (n == 4 && !showBulk()) {
            EditInfo ei = new EditInfo("", 0, -1, -1);
        ...
        if (n == 4 && showBulk()) {
            EditInfo ei = new EditInfo("", 0, -1, -1);
            ei.checkbox = new JCheckBox("Simulate Body Diode", (flags & FLAG_BODY_DIODE) != 0);
            return ei;
        }
        if (n == 5 && doBodyDiode()) {
            EditInfo ei = new EditInfo("", 0, -1, -1);
            ei.checkbox = new JCheckBox("Body Terminal", (flags & FLAG_BODY_TERMINAL) != 0);
            return ei;
        }
    ...
    public void setEditValue(int n, EditInfo ei) {
        ...
        if (n == 2) {
            globalFlags = (!ei.checkbox.isSelected()) ? (globalFlags | FLAG_HIDE_BULK)
                    : (globalFlags & ~(FLAG_HIDE_BULK | FLAG_DIGITAL));
            setPoints();
            ei.newDialog = true;
        }
        if (n == 3) {
            flags = (ei.checkbox.isSelected()) ? (flags | FLAG_FLIP)
                    : (flags & ~FLAG_FLIP);
            setPoints();
        }
        if (n == 4 && !showBulk()) {
            globalFlags = (ei.checkbox.isSelected()) ? (globalFlags | FLAG_DIGITAL)
