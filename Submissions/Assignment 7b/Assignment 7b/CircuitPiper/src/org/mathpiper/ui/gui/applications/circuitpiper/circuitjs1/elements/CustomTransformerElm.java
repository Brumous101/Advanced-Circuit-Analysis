...
//	    width = max(32, abs(yy-y));
    ...
    public void draw(Graphics g) {
        ...
        for (i = 0; i != coilCount; i++) {
            int n = coilNodes[i];
            setVoltageColor(g, nodeVoltages[n]);
            setPowerColor(g, coilCurrents[i] * (nodeVoltages[n] - nodeVoltages[n + 1]));
            drawCoil(g, (i >= primaryCoils ? -6 : 6), nodeTaps[n], nodeTaps[n + 1], nodeVoltages[n], nodeVoltages[n + 1]);
            if (dots != null) {
    ...
    public void reset() {
        ...
        for (i = 0; i != nodeCount; i++) {
            nodeVoltages[i] = nodeCurrents[i] = nodeCurCounts[i] = 0;
        }
    }
    double xformMatrix[][];
    ...
    public void startIteration() {
        ...
        for (i = 0; i != coilCount; i++) {
            ...
            if (isTrapezoidal()) {
                ...
                for (j = 0; j != coilCount; j++) {
                    int n = coilNodes[j];
                    double voltdiff = nodeVoltages[n] - nodeVoltages[n + 1];
    ...
    public void getInfo(String arr[]) {
        ...
        for (i = 0; i != coilCount; i++) {
            ...
            int ni = coilNodes[i];
            arr[2 + i * 2] = "Vd" + (i + 1) + " = " + getVoltageText(nodeVoltages[ni] - nodeVoltages[ni + 1]);
        ...
        if (n == 1) {
            ...
            if (s != description) {
                if (!parseDescription(s)) {
                    parseDescription(description);
                    JOptionPane.showMessageDialog(null, "Parse error in description");
                } else {
                    description = s;
                }
                setPoints();
