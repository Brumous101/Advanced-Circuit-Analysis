...
public class LEDArrayElm extends ChipElm {
    ...
    public void setupPins() {
        ...
        int i;
        for (i = 0; i != sizeX; i++) {
            pins[i] = new Pin(i, SIDE_SOUTH, "");
        }
        for (i = 0; i != sizeY; i++) {
            pins[i + sizeX] = new Pin(i, SIDE_WEST, "");
        }
    ...
    public void doStep() {
        ...
        for (iy = 0; iy != sizeY; iy++) {
            for (ix = 0; ix != sizeX; ix++, i++) {
                diodes[i].doStep(nodeVoltages[sizeX + iy] - nodeVoltages[ix]);
            }
        }
    }
    ...
    void calculateCurrent() {
        ...
        for (iy = 0; iy != sizeY; iy++) {
            double cur = 0;
            for (ix = 0; ix != sizeX; ix++, i++) {
                currents[i] = diodes[i].calculateCurrent(nodeVoltages[sizeX + iy] - nodeVoltages[ix]);
                cur += currents[i];
