...
public class SevenSegElm extends ChipElm {
    ...
    public static final int ES_DP = 1;
    public static final int ES_COLON = 2;
    ...
    public void setupPins() {
        ...
        for (; i != segmentCount; i++) {
            pins[i] = new Pin(s++, backwardCompatibility ? SIDE_SOUTH : SIDE_EAST, Character.toString((char) ('a' + i)));
        }
    ...
    public void doStep() {
        ...
        int i;
        for (i = 0; i != segmentCount; i++) {
            diodes[i].doStep(diodeDirection * (nodeVoltages[i] - nodeVoltages[commonPin]));
        }
    }
    ...
    public void draw(Graphics g) {
        ...
        int spy = spx * 2;
        int xl = x1 + cspc + sizeX * cspc - spx;
        int yl = y1 - cspc + sizeY * cspc - spy;
        int i;
    ...
    void calculateCurrent() {
        ...
        for (i = 0; i != segmentCount; i++) {
            pins[i].current = -diodeDirection * diodes[i].calculateCurrent(diodeDirection * (nodeVoltages[i] - nodeVoltages[commonPin]));
    ...
    public EditInfo getEditInfo(int n) {
        if (n == 2) {
            ...
            ei.comboBox.addItem("7 Segment");
            ei.comboBox.addItem("14 Segment");
            ei.comboBox.addItem("16 Segment");
            ei.comboBox.setSelectedItem(baseSegmentCount == 7 ? 0 : baseSegmentCount == 14 ? 1 : 2);
            return ei;
        }
        if (n == 3) {
            EditInfo ei = new EditInfo("Extra Segment", 0, -1, -1);
            ei.comboBox = new JComboBox();
            ei.comboBox.addItem("None");
            ei.comboBox.addItem("Decimal Point");
            ei.comboBox.addItem("Colon");
            ei.comboBox.setSelectedItem(extraSegment);
            return ei;
        }
        if (n == 4) {
            EditInfo ei = new EditInfo("Diodes", 0, -1, -1);
            ei.comboBox = new JComboBox();
            ei.comboBox.addItem("Common Cathode");
            ei.comboBox.addItem("Common Anode");
            ei.comboBox.addItem("None (logic inputs)");
            ei.comboBox.setSelectedItem(diodeDirection == 1 ? 0 : diodeDirection == -1 ? 1 : 2);
            return ei;
        }
    ...
    public void setEditValue(int n, EditInfo ei) throws Exception {
        ...
        if (n == 3) {
            extraSegment = ei.comboBox.getSelectedIndex();
            setPinCount();
            return;
        }
        if (n == 4) {
