...
public class RingCounterElm extends ChipElm {
    ...
    public void setupPins() {
        ...
        pins[0] = new Pin(1, SIDE_WEST, "");
        pins[0].clock = true;
        pins[1] = new Pin(sizeX - 1, SIDE_SOUTH, "R");
        pins[1].bubble = true;
        int i;
        for (i = 0; i != bits; i++) {
            int ii = i + 2;
            pins[ii] = new Pin(i, SIDE_NORTH, "Q" + i);
    ...
    void execute() {
        ...
        // if we just loaded then the nodeVoltages[] array is likely to be all zeroes, which might force us to do a reset, so defer execution until the next iteration
        if (justLoaded) {
            justLoaded = false;
            return;
        }
