...
public final class CircuitPanel extends JPanel {
    ...
    public void newTempComponent(String name, int headX, int headY) throws Exception {
        ...
        myTempComponent = currentCircuit.newComponent(name, headX, headY, null);
    ...
    public Terminal getNearTerminal()
    {
        if(isNearTerminal())
        {
            return currentCircuit.terminals.get(new Point(nearestGridPointXPixels(), nearestGridPointYPixels()));
        }
    ...
    public void deleteAllScopePanels()
    {
        if(scopesPanel != null)
        {
            scopesPanel.removeAll();
            scopesPanel.invalidate();
