...
public class TransLineElm extends CircuitElm {
    ...
    public void drag(int xx, int yy) {
        xx = sim.snapGrid(xx);
        yy = sim.snapGrid(yy);
        int w1 = max(sim.gridSize, abs(yy - y1));
        int w2 = max(sim.gridSize, abs(xx - x1));
        if (w1 > w2) {
            xx = x1;
            width = w2;
        } else {
    ...
    public void startIteration() {
        ...
        voltageL[ptr] = nodeVoltages[2] - nodeVoltages[0] + nodeVoltages[2] - nodeVoltages[4];
        voltageR[ptr] = nodeVoltages[3] - nodeVoltages[1] + nodeVoltages[3] - nodeVoltages[5];
        //System.out.println(nodeVoltages[2] + " " + nodeVoltages[0] + " " + (nodeVoltages[2]-nodeVoltages[0]) + " " + (imped*current1) + " " + voltageL[ptr]);
    ...
    public void doStep() {
        ...
        if (Math.abs(nodeVoltages[0]) > 1e-5 || Math.abs(nodeVoltages[1]) > 1e-5) {
    ...
    //public double getVoltageDiff() { return nodeVoltages[0]; }
