...
public class DACElm extends ChipElm {
    ...
    public void setupPins() {
        ...
        int i;
        for (i = 0; i != bits; i++) {
            pins[i] = new Pin(bits - 1 - i, SIDE_WEST, "D" + i);
        }
        pins[bits] = new Pin(0, SIDE_EAST, "O");
        pins[bits].output = true;
        pins[bits + 1] = new Pin(sizeY - 1, SIDE_EAST, "V+");
        allocNodes();
    }
    ...
    public void doStep() {
        ...
        int i;
        for (i = 0; i != bits; i++) {
            if (nodeVoltages[i] > 2.5) {
                ival |= 1 << i;
            }
        }
        int ivalmax = (1 << bits) - 1;
        double v = ival * nodeVoltages[bits + 1] / ivalmax;
