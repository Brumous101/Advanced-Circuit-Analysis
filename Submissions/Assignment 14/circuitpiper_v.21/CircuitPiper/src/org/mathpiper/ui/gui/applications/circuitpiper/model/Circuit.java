...
import org.mathpiper.ui.gui.applications.circuitpiper.model.components.active.Ground;
...
*/
    ...
    public Component newComponent(String typeAndUID, int headX, int headY, Stack componentAttributes, StringTokenizer elmAttributes, CircuitPanel circuitPanel) throws Exception
    {
        ...
        {
            // These coordinates are eventually overwritten by
            // "Component.setHead" and "Component.setTail".
        ...
        else if (componentType.startsWith("Ground"))
        {
            if (componentAttributes == null)
            {
                newComponent = new Ground(headX, headY, uid, this);
            }
            else
            {
                newComponent = new Ground(headX, headY, uid, componentAttributes, this);
            }
            ...
            if (isCirSim)
            {
                if (elmAttributes == null)
                {
                    cirElm = new GroundElm(headX, headY);
                }
                else
                {
                    cirElm = new GroundElm(x1, y1, x2, y2, flags, elmAttributes);
                }
        ...
        else if (componentType.startsWith("VCCS"))
        {
            ...
            if (isCirSim)
            {
                if (elmAttributes == null)
                {
                    cirElm = new VCCS2Elm(headX, headY);
                }
                else
                {
        ...
        else if (componentType.startsWith("CCVS"))
        {
            ...
            if (isCirSim)
            {
                if (elmAttributes == null)
                {
                    //cirElm = new CCVS2Elm(headX, headY);
                }
                else
                {
                    /*
                    CCVS2Elm ccvs2Elm = new CCVS2Elm(x1, y1, x2, y2, flags, elmAttributes);
                    cirElm = ccvs2Elm;
                    if (ccvs2Elm.wireControl0 != null)
                    {
                        cirSim.elmList.add(ccvs2Elm.wireControl0);
                    }
                    if (ccvs2Elm.wireControl1 != null)
                    {
                        cirSim.elmList.add(ccvs2Elm.wireControl1);
                    }
*/
                }
