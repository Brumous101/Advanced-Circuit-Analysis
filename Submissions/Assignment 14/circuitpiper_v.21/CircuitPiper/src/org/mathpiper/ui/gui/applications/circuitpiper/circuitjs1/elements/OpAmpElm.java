...
public class OpAmpElm extends CircuitElm {
    ...
    public OpAmpElm(int xa, int ya, int xb, int yb, int f,
        ...
        try {
            ...
            nodeVoltages[0] = Double.parseDouble(st.nextToken());
            nodeVoltages[1] = Double.parseDouble(st.nextToken());
    ...
    public String dump() {
        flags |= FLAG_GAIN;
        return super.dump() + " " + maxOut + " " + minOut + " " + gbw + " " + nodeVoltages[0] + " " + nodeVoltages[1] + " " + gain;
    }
    ...
    public void getInfo(String arr[]) {
        ...
        double vo = Math.max(Math.min(nodeVoltages[2], maxOut), minOut);
    ...
    public void doStep() {
        double vd = nodeVoltages[1] - nodeVoltages[0];
        if (Math.abs(lastvd - vd) > .1) {
            sim.converged = false;
        } else if (nodeVoltages[2] > maxOut + .1 || nodeVoltages[2] < minOut - .1) {
            sim.converged = false;
        }
        double x = 0;
        int vn = sim.nodeList.size() + voltSource;
        ...
        //System.out.println("opamp " + vd + " " + nodeVoltages[2] + " " + dx + " "  + x + " " + lastvd + " " + sim.converged);
        ...
        lastvd = vd;
        /*if (sim.converged)
	      System.out.println((nodeVoltages[1]-nodeVoltages[0]) + " " + nodeVoltages[2] + " " + initvd);*/
    }
