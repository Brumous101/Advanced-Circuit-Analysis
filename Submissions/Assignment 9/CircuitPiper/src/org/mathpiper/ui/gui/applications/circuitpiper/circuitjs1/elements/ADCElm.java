...
public class ADCElm extends ChipElm {
    ...
    public void setupPins() {
        ...
        int i;
        for (i = 0; i != bits; i++) {
            pins[i] = new Pin(bits - 1 - i, SIDE_EAST, "D" + i);
            pins[i].output = true;
        }
        pins[bits] = new Pin(0, SIDE_WEST, "In");
        pins[bits + 1] = new Pin(sizeY - 1, SIDE_WEST, "V+");
        allocNodes();
    }
    ...
    void execute() {
        ...
        double val = imax * nodeVoltages[bits] / nodeVoltages[bits + 1]; // + .5;
        int ival = (int) val;
