...
public class AudioOutputElm extends CircuitElm {
    ...
    public void getInfo(String arr[]) {
        ...
        arr[2] = "start = " + getUnitText(dataFull ? sim.simulationTime - duration : dataStart, "s");
    ...
    public void stepFinished() {
        dataSample += nodeVoltages[0];
        dataSampleCount++;
        if (sim.simulationTime >= nextDataSample) {
    ...
    void setDataCount() {
        ...
        dataStart = sim.simulationTime;
        dataPtr = 0;
        dataFull = false;
        sampleStep = 1. / samplingRate;
        nextDataSample = sim.simulationTime + sampleStep;
    }
	...
	public EditInfo getEditInfo(int n) {
		...
		for (i = 0; i != samplingRateChoices.length; i++) {
		    ei.comboBox.addItem(samplingRateChoices[i] + "");
    ...
    public void setEditValue(int n, EditInfo ei) {
	    ...
	    if (n == 1) {
		int nsr = samplingRateChoices[ei.comboBox.getSelectedIndex()];
		if (nsr != samplingRate) {
		    samplingRate = nsr;
...
//        console.log(url);
    ...
    void play() {
        ...
        if (ct * sampleStep < .05) {
            JOptionPane.showMessageDialog(null, sim.LS("Audio data is not ready yet.  Increase simulation speed to make data ready sooner."));
            return;
        }
