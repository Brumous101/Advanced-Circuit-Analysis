...
public class OpAmpRealElm extends CompositeElm {
    ...
    public static final int MODEL_741 = 0;
    public static final int MODEL_324 = 1;
    ...
    public OpAmpRealElm(int xx, int yy) throws Exception {
    ...
    public OpAmpRealElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) throws Exception {
    ...
    private void init741() throws Exception {
    ...
    private void init324() throws Exception {
    ...
    public void draw(Graphics g) {
        ...
        setVoltageColor(g, nodeVoltages[3]);
        drawThickLine(g, rail1p[0], rail1p[1]);
        setVoltageColor(g, nodeVoltages[4]);
    ...
    public void getInfo(String arr[]) {
        ...
        arr[0] = "op-amp (" + type + ")";
        arr[1] = "V+ = " + getVoltageText(nodeVoltages[1]);
        arr[2] = "V- = " + getVoltageText(nodeVoltages[0]);
        arr[3] = "Vout = " + getVoltageText(nodeVoltages[2]);
    ...
    public EditInfo getEditInfo(int n) {
        ...
        if (n == 2) {
            ...
            ei.checkbox = new JCheckBox("Swap Inputs", (flags & FLAG_SWAP) != 0);
            return ei;
        }
        if (n == 3) {
            EditInfo ei = new EditInfo("<a href=\"opampreal.html\" target=\"_blank\">Model</a>", modelType);
            ei.comboBox = new JComboBox();
            ei.comboBox.addItem("LM741");
            ei.comboBox.addItem("LM324");
            ei.comboBox.setSelectedItem(modelType);
            return ei;
    ...
    public void setEditValue(int n, EditInfo ei) throws Exception {
        ...
        if (n == 3) {
            modelType = ei.comboBox.getSelectedIndex();
            capValue = 0;
            initModel();
        }
    }
