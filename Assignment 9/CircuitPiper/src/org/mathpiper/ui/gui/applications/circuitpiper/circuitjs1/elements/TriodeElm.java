...
public class TriodeElm extends CircuitElm {
    ...
    public void draw(Graphics g) {
        ...
        setPowerColor(g, currentp * (nodeVoltages[0] - nodeVoltages[2]));
        drawThickLine(g, plate[0], plate[1]);
        drawThickLine(g, plate[2], plate[3]);
        // draw grid
        setVoltageColor(g, nodeVoltages[1]);
        setPowerColor(g, currentg * (nodeVoltages[1] - nodeVoltages[2]));
        int i;
        for (i = 0; i != 8; i += 2) {
    ...
    double getPower() {
        return (nodeVoltages[0] - nodeVoltages[2]) * current;
    }
